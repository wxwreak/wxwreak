...
export default function ProjectsRow({ number, title, subtitle, description, url, tags, role, period }) {
  return (
    <a 
      href={url} 
      target="_blank" 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-baseline gap-4 md:gap-6 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-2xl md:text-3xl font-semibold text-white/90 group-hover:text-white transition-colors duration-300">
              {title}
            </h3>
            {subtitle && (
              <>
        ...
        <div className="flex flex-col sm:items-end">
          ...
          {period && (
            <span className="font-mono text-[11px] text-gray-600 mt-0.5">
              [ {period} ]
            </span>
          )}
        </div>
